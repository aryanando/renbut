...
class RemunController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('transaksi.remun.index2', compact('remun'));
    }
    ...
    public function savetarget(Request $request)
    {
        ...
        if ($unit_id == 999) {
            ...
            return redirect('remunbaru')->with('flash_message', 'Target Tersimpan!');
        }
    ...
    public function genrekan($pegawai_id = null, $user_id = null)
    {
        ...
        if ($pangkat != 2 && $pangkat != 4 && $pangkat != 5 ) {
    ...
    public function genconsolesave($pegawai_id, $user_id)
    {
        ...
        // $cekunit = dd(DB::table('pegawai')->select('unit_id')->where('id',$pegawai_id));
        // $cekunit = Pegawai::where('id', '=', $pegawai_id)->where(['unit_id' => $unit_id])->orderby('nama')->get();
            $requestData = [    
            "rekan1" => ["reliability" => ["0"],
                "assurance" => ["0", "0"],
                "tangibility" => ["0"],
                "empathy" => ["0", "0"],
                "responsiveness" => ["0", "0"],
        ...
        DB::table('ibadah')->where(['periode' => $periode, 'user_id' => $user_id])->delete();
...
// 
    ...
    public function grading(){
            ...
            $employees = DB::table('pegawai')->select('id', 'tmt_kerja')->get();

            // Iterate through the employees and update their grading
            foreach ($employees as $employee) {
                // Calculate the tenure in days
                $dateOfEntry = \Carbon\Carbon::parse($employee->tmt_kerja);
                $currentDate = \Carbon\Carbon::now();
                $banding = $currentDate->diffinMonths($dateOfEntry);

                // Determine the grading based on the tenure
                switch ($banding) {
                    case ($banding < 12):
                        $grading = 5;
                        break;
                    case ($banding > 12 && $banding <=24):
                        $grading = 4;
                        break;
                    case ($banding > 24 && $banding <=36):
                        $grading = 3;
                        break;
                    case ($banding > 36 && $banding <=60):
                        $grading = 2;
                        break;
                    case ($banding >= 84):
                        $grading = 1;
                ...
                DB::table('grade_karyawan')->where('pegawai_id', $employee->id)->update(['grade' => $grading]);

            }

        return redirect('remun/tiga60')->with('flash_message', 'Grading sudah diupdate!');

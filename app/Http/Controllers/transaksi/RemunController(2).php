...
class RemunController extends Controller
{
    ...
    public function genrekan($pegawai_id = null, $user_id = null)
    {
        ...
        if ($pangkat != 2 && $pangkat != 4) {
            $b = Pegawai::join('users', 'users.pegawai_id', '=', 'pegawai.id')->select(DB::raw('pegawai.*'))->where('atasan', '<', 2)->where(['unit_id' => $unit_id])->orderby('nama')->get();
        } else {
            $b = Pegawai::join('users', 'users.pegawai_id', '=', 'pegawai.id')->join('units', 'units.id', '=', 'pegawai.unit_id')->select(DB::raw('pegawai.*'))->where(['atasan' => 2, 'units.remun' => $unit->remun])->orderby('nama')->get();
        }

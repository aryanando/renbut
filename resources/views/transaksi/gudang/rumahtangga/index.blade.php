...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-archive"></i> Rumah Tangga</li>
    </ol>
@endsection
                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="box-title" style="margin-top:-5px">
                            <form method="post" action="{{ url('gudang/rumahtangga/toexcel') }}">
                                {{ csrf_field() }}
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                    <form method="POST" action="{{ url('gudang/rumahtangga') }}" accept-charset="UTF-8" class="" enctype="multipart/form-data">
                            <div class="box-body">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-3 {{ $errors->has('uraian') ? 'has-error' : ''}}">
                                        <label for="uraian" class="control-label">{{ 'Uraian' }}</label>
                                        <input type="hidden" name="uraian" type="text" id="uraian" value="{{ isset($rumahtangga->uraian) ? $rumahtangga->uraian : ''}}" data-tags="true">
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('satuan') ? 'has-error' : ''}}">
                                        ...
                                        <input type="hidden" name="satuan" type="text" id="satuan" value="{{ isset($rumahtangga->satuan) ? $rumahtangga->satuan : ''}}" data-tags="true">
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('harga') ? 'has-error' : ''}}">
                                        ...
                                        <input class="form-control" name="harga" type="number" id="harga" value="{{ isset($rumahtangga->harga) ? $rumahtangga->harga : ''}}" required>
                        ...
                        <div class="table-responsive">
                            <table class="table" id="table3">
                                ...
                                @foreach($rumahtangga as $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{--
                                            <a href="javascript:void(0)" title="Edit rumahtangga" class="edit">
                                                ...
                                                <a href="{{ url('gudang/rumahtangga/'.$item->id.'/edit') }}" title="Edit rumahtangga" class="edit">
                                        ...
                                        {{--   @if($item->id && $item->unit_id == Auth::user()->unit)
                                            <form method="POST" action="{{ url('/transaksi/rumahtangga' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn-danger btn-sm" title="Delete rumahtangga" ><i class="fa fa-trash-o" aria-hidden="true"></i></button>
                                            </form>
                                            @endif--}} 
                                        </td>
                                    </tr>
                            ...
                            {{-- <div class="pagination-wrapper"> {!! $rumahtangga->appends(['search' => Request::get('search')])->render() !!} </div> --}}
                        </div>
                    </div>
                    <!-- /.box-body -->
                </div>

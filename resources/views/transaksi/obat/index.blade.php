...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-file"></i> Obat</li>
    </ol>
@endsection
                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="box-title row">
                            <form class="col-md-3" method="post" action="{{ url('/transaksi/obat/toexcel') }}">
                                {{ csrf_field() }}
                                <button class="btn btn-success btn-sm" type="submit"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</button>
                            </form>
                            <form class="col-md-6" method="post" action="{{ url('/transaksi/obat/periodesebelum') }}">
                                {{ csrf_field() }}
                                <button class="btn btn-warning btn-sm" type="submit"><i class="fa fa-history" aria-hidden="true"></i> Copy Periode Sebelumnya</button>
                            </form>
                        </div>
                        <div class="box-tools">
                            <form method="GET" action="{{ url('/transaksi/obat') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                        <form method="POST" action="{{ url('/transaksi/obat') }}" accept-charset="UTF-8" class="" enctype="multipart/form-data">
                            <div class="box-body">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-3 {{ $errors->has('periode') ? 'has-error' : ''}}">
                                        ...
                                        {{-- <input class="form-control" name="periode" type="month" id="periode" value="{{ isset($obat->periode) ? substr($obat->periode,0,7) : '0001-01'}}" required> --}}
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        @endphp
                                        <h3 class="pull-right">
                                            Total Harga: <b>Rp {{ number_format($sum,0,",",".") }}</b>
                                        </h3>
                                    </div>
                                </div>
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('jumlah') ? 'has-error' : ''}}">
                                        ...
                                        <input class="form-control" name="jumlah" type="number" id="jumlah" value="{{ isset($obat->jumlah) ? $obat->jumlah : ''}}" required>
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('total') ? 'has-error' : ''}}">
                                        ...
                                        <input class="form-control" type="number" id="total" value="{{ isset($obat->total) ? $obat->total : ''}}" readonly required>
                        ...
                        <div class="table-responsive">
                            <table class="table" id="table">
                                ...
                                @foreach($obat as $item)
                                    <tr id="{{ 'ur-'.str_replace(" ","-",strtolower($item->uraian)) }}">
                                        ...
                                        <td>
                                            ...
                                            <form method="POST" action="{{ url('/transaksi/obat' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn-danger btn-sm" title="Delete Obat" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
                                            </form>
                                            @endif
                                        </td>
                                    </tr>

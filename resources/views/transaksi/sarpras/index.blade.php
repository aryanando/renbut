...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-file"></i> Sarpras</li>
    </ol>
@endsection
                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="box-title" style="margin-top:-5px">
                            <form method="post" action="{{ url('/transaksi/sarpras/toexcel') }}">
                                {{ csrf_field() }}
                                <button class="btn btn-success btn-sm" type="submit"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</button>
                            </form>
                        </div>
                        <div class="box-tools">
                            <form method="GET" action="{{ url('/transaksi/sarpras') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                        <form method="POST" action="{{ url('/transaksi/sarpras') }}" accept-charset="UTF-8" class="" enctype="multipart/form-data">
                            <div class="box-body">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="form-group col-md-3 {{ $errors->has('periode') ? 'has-error' : ''}}">
                                        ...
                                        {{-- <input class="form-control" name="periode" type="month" id="periode" value="{{ isset($sarpras->periode) ? substr($sarpras->periode,0,7) : '0001-01'}}" required> --}}
                                ...
                                <div class="row">
                                    <div class="form-group col-md-3 {{ $errors->has('uraian') ? 'has-error' : ''}}">
                                        ...
                                        <input type="hidden" name="uraian" type="text" id="uraian" value="{{ isset($sarpras->uraian) ? $sarpras->uraian : ''}}" data-tags="true">
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('satuan') ? 'has-error' : ''}}">
                                        ...
                                        <input type="hidden" name="satuan" type="text" id="satuan" value="{{ isset($sarpras->satuan) ? $sarpras->satuan : ''}}" data-tags="true">
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('jumlah') ? 'has-error' : ''}}">
                                        ...
                                        <input class="form-control" name="jumlah" type="number" id="jumlah" value="{{ isset($sarpras->jumlah) ? $sarpras->jumlah : ''}}" required>
                                    ...
                                    <div class="form-group col-md-2 {{ $errors->has('harga') ? 'has-error' : ''}}">
                                        ...
                                        <input class="form-control" name="harga" type="number" id="harga" value="{{ isset($sarpras->harga) ? $sarpras->harga : ''}}" required>
                                        {!! $errors->first('harga', '<p class="help-block">:message</p>') !!}
                                    </div>
                                    <div class="form-group col-md-2 {{ $errors->has('total') ? 'has-error' : ''}}">
                                        <label for="total" class="control-label">{{ 'Total' }}</label>
                                        <input class="form-control" type="number" id="total" value="{{ isset($sarpras->total) ? $sarpras->total : ''}}" readonly required>
                        ...
                        <div class="table-responsive">
                            <table class="table" id="table">
                                ...
                                @foreach($sarpras as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0)" title="Edit Sarpras" class="edit">
                                            ...
                                            <form method="POST" action="{{ url('/transaksi/sarpras' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn-danger btn-sm" title="Delete Sarpras" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
                                            </form>
                                            @endif
                                        </td>
                                    </tr>
                            ...
                            {{-- <div class="pagination-wrapper"> {!! $sarpras->appends(['search' => Request::get('search')])->render() !!} </div> --}}
                        </div>
                    </div>
                    <!-- /.box-body -->
                </div>

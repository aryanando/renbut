                            ...
                            <tbody>
                                ...
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    ...
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        ...
                                        number_format($item->atasan) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        number_format($item->rekanan)}}</td>
                                        <!-- round(number_format($item->raterdes_rekan)*100/150) }}</td> -->
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        number_format($item->diri)}}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                    ...
                    <div class="table-responsive" id="dvDataDok">
                        <table class="table" style="margin-bottom: 0px">
                            <thead>
                                <tr>
                                    <th colspan="3" bgcolor="#F3F3F3">Remun Periode: <b>{{ date('M-Y') }}</b></th>
                                    <th colspan ="2" bgcolor="#F3F3F3" style="text-align: center">
                                        Target Unit
                                        <a href="{{ url('/remun/target') }}" title="Edit Target" class="edit">
                                            <button class="btn btn-primary btn-xs" name="edit_t"><i
                                                    class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
                                ...
                                <tr>
                                    ...
                                    <th bgcolor="#F3F3F3">Nama</th>
                                    <th bgcolor="#F3F3F3">Unit</th>
                                    <th bgcolor="#F3F3F3">Atasan</th>
                                    <th bgcolor="#F3F3F3">Diri</th>
                                    <!-- <th bgcolor="#F3F3F3">Desrater</th>
                                    <th bgcolor="#F3F3F3">%</th> -->
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @endphp
                                @foreach($remun as $item)
                                @if(in_array($item->unit_id, [Auth::user()->unit]))
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    <td>{{ ++$i }}</td>
                                    <td style="white-space: nowrap">{{ strtoupper($item->nama) }}</td>
                                    <td>{{ ucfirst($item->keterangan) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        number_format($item->atasan) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :

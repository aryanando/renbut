    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                ...
                <div class="box-body">
                <h3>Dokter</h3>
                    <div class="table-responsive" id="dvDataDok">
                        <table class="table" style="margin-bottom: 0px">
                            <thead>
                                <tr>
                                    ...
                                    <th colspan="{{ Auth::user()->unit == 999 ? 5 : 1 }}" bgcolor="pink">Komponen
                                        DesRater</th>
                            ...
                            <tbody>
                                ...
                                @if( $item->jenis_remun == 1200 && in_array($item->unit_id, [1003]))
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    ...
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        ...
                                        round(number_format($item->raterdes_atasan)*100/75) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        round(number_format($item->raterdes_rekan)*100/150) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        round(number_format($item->raterdes_diri)*100/75) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        number_format($item->grading) }}</td>
                                    @endif
                    ...
                    <div class="table-responsive" id="dvData1">
                        <table class="table" style="margin-bottom: 0px">
                                    ...
                                    <th bgcolor="orange" style="text-align: center">
                                        <a href="{{ url('/remun/target') }}" title="Edit Target" class="edit">
                                            <button class="btn btn-primary btn-xs" name="edit_t"><i
                                                    class="fa fa-pencil-square-o" aria-hidden="true">Target Unit</i></button>
                                        </a>
                                    </th>
                                    <th colspan="3" bgcolor="#16D39D" style="text-align: center">Evaluasi Ibadah</th>
                                    <th bgcolor="#16D39D" style="text-align: center">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach($remun as $item)
                                @if(!in_array($item->unit_id, [1003]) && $item->jenis_remun == 1200 || $item->jenis_remun == 1000 )
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    <td>{{ ++$i }}</td>
                                    <td style="white-space: nowrap">{{ strtoupper($item->nama) }}</td>
                                    <td>{{ ucfirst($item->keterangan) }}</td>
                                    ...
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        floor(number_format($item->raterdes_atasan)*100/75) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        floor(number_format($item->raterdes_rekan)*100/150) }}</td>
                                        <!-- round(number_format($item->raterdes_rekan)*100/150) }}</td> -->
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        floor(number_format($item->raterdes_diri)*100/75) }}</td>
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : '#FFFFD5' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '0' :
                                        number_format($item->grading) }}</td>
                                    @endif
                                    <td bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'pink' }}"
                                        style="text-align: right">{{ $item->cuti >= date('Y-m-t') ? '' : $item->desrater + $item->grd
                                        }}</td>
                    ...
                    <div class="table-responsive" id="dvData2">
                        <table class="table" style="margin-bottom: 0px">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach($remun as $item)
                                @if( $item->jenis_remun == 800 && !in_array($item->unit_id, [16,21]))
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    <td>{{ ++$i }}</td>
                                    <td style="white-space: nowrap">{{ strtoupper($item->nama) }}</td>
                                    <td>{{ ucfirst($item->keterangan) }}</td>
                    ...
                    <div class="table-responsive" id="dvData3">
                        <table class="table" style="margin-bottom: 0px">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach($remun as $item)
                                @if(in_array($item->jenis_remun, [500, 700]))
                                <tr bgcolor="{{ $item->cuti >= date('Y-m-t') ? 'yellow' : 'white' }}">
                                    <td>{{ ++$i }}</td>
                                    <td style="white-space: nowrap">{{ strtoupper($item->nama) }}</td>
                                    <td>{{ ucfirst($item->keterangan) }}</td>
                ...
                <div class="box-footer">
                    <h2>Export Excel</h2>
                    <a class="btn btn-success" id="btnExportDok"><i class="fa fa-file-excel-o"></i>Dokter</a>
...
@section('jquery')
$("#btnExportDok").click(function (e) {
$(this).attr({
'download': "Dokter.xls",
'href': 'data:application/csv;charset=utf-8,' + encodeURIComponent( $('#dvDataDok').html())
})
});
